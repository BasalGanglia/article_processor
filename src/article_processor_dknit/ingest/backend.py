...
class Worker(Process):
    """
    Worker is a process that gets items from a message queue and processes them"""

    def __init__(self, inq: QueueWrapper, outq: QueueWrapper, cache_size: int = 25000):
        self.iq: QueueWrapper = inq
        self.oq: QueueWrapper = outq
        self._cache_size: int = cache_size
        self.reset_cache()

    def count(self, incr_num: int = 0) -> int:
        """Returns the number of items processed by this worker"""
    ...
    def flush_cache(self):
        for post in self._cache.values():
            self.oq.put_many(post._transform_for_database())
        self.reset_cache()

    def run(self):
        # Register the shutdown handler for this process
        signal.signal(signal.SIGTERM, self.shutdown)
        # Only the worker processed need to use hte data processor
        # The data processor uses Spacy for its processing.
        # Spacy can take up memory when loaded.
        # So we only load it in the worker processes.
        processor = DataProcessor()
        # self.iq.get() blocks until there is an item in the queue
...
class Saver(Process):
    """
    Saver pulls messages off the queue and passes the message and client ot the persist_fn
    """
...
def start_processes(
    proc_num: int, proc: Process, proc_args: List[object]
) -> List[Process]:
    """instantiates and starts the given process"""

    log.info(f"Starting {proc_num} {proc.__name__} processes")
...
def main():
    ...
    parser_arguments = [
        (
            "--iproc_num",
            {"help": "number of input queue workers", "default": pcount, "type": int},
        ),  # noqa
        (
            "--oproc_num",
            {"help": "number of output queue workers", "default": pcount, "type": int},
        ),  # noqa
        (
            "--iport",
            {
                "help": "input queue port cross proc messaging",
                "default": 50_000,
                "type": int,
            },
        ),  # noqa
        (
            "--no_persistence",
            {"help": "disable database persistence", "action": "store_true"},
        ),  # noqa
        (
            "--agg_cache_size",
            {"help": "aggregator cache size", "default": 25_000, "type": int},
        ),  # noqa
    ]
